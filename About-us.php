<html>
<head>
	<?php include("head.php"); ?>
</head>
<body>
<?php include("new-header.php"); ?>
<div class="device-hide"><?php include("social-links.php"); ?></div>
<div class="banner-section">
	<img src="content/about_main_image.jpg" alt="clinical toxicology" />
	<div class="banner-caption">
		<h1><span class="text-light-green">About </span><span class="text-dark-green">us</span></h1>
		<p class="banner-description">
			<b class="font-size-62pt">Acutis Diagnostics</b> uses the most advance Technology and applies  the most innovative thinking to medication monitoring and testing  for illictic drugs.
		</p>
	</div>
</div><!-- Well, this is the end of the Banner of the Project Ver2.0 -->

<div class="dez-col-100 no-shadow">
	<div class="dez-col-55 padding-5 no-shadow">
		<p class="padding-left-20 device-center text-light-black">
			State-of-the art  instrumentation and technologies are employed to determine results and then each sample is triple checked  to confirm the accuracy of our Data. This information is then communicated in our proprietary <span class="special-text">728 Patient Reports.</span>
		</p>
	</div>
	<div class="dez-col-25 padding-5 no-shadow">
		<center><img src="content/about_icon_paragraph_1.jpg" alt="illicit drugs" class="project-image-normal padding-right-20 float-left" /></center>
	</div>
</div><!-- this is the end of the first section of content and its image. -->

<div class="dez-col-100 no-shadow">
	<div class="dez-col-25 padding-5 no-shadow">
		<center><img src="content/about_icon_paragraph_2.jpg" alt="medication" class="project-image-normal padding-left-20 float-right" /></center>
	</div>
	<div class="dez-col-55 padding-left-2 padding-top-5 no-shadow">
		<p class="device-center text-light-black">Whether taken in hand or read on a screen, our <span class="special-text">728 Patient Reports</span> allow Physicians, Clinicians, Counselors and Others to appraise a glance the patient's data. In Possession of the facts, doctor can make the best possible medical decisions.<br><br>Decisions that will produce the best outcomes, in the most-timely fashion.</p>
	</div>
</div><!-- This is the end of the second section of image  and its content. -->

<div class="dez-col-100 no-shadow">
		<center><img src="content/about_green_dots.jpg" alt="innovative technologies" class="project-image-small" /></center>
</div>

<div class="dez-col-100 no-shadow">
	<center>
	<h2 class="font-34"><span class="text-light-green">Speak to a</span><span class="text-dark-green"> toxicology</span></h2>
	<p class="text-light-green">For a Patient's report or other general Support</p>
	<p class="special-text no-border"><b>1-844-522-8847</b></p>
	<img src="content/about_arrow.jpg" alt="clinical judgement" class="mobile-width-auto" />
	<p class="text-dark-green"><b>Choose Option 1</b></p>
	</center>
	<h1 class="padding-left-10 device-center"><span class="text-light-green">Our</span> Team</h1>
	
	<div class="dez-col-100 no-shadow">
	<div class="dez-col-30 padding-5 no-shadow">
		<center><img src="content/about_icon_our_team_2.jpg" alt="medication monitoring" class="project-image-normal padding-left-20 padding-top-20" /></center>
	</div>
	<div class="dez-col-50 padding-top-5 no-shadow">
		<p class="device-center text-light-black">At Acutis, we combine hard science practiced by our scientists and technologists, with the soft skills possessed by our support staff and sales specialists. We believe that rigorous technological processes and best practices are only fully realized when their benefits are accessible to every member of the team. Toward that end, we have built a company with no gaps. No bureaucracy, No insurmountable hierarchy.<br><br>
		From our CEO engaged in strategic planning and our senior scientists in the lab, to sale specialists on the road, every employee is fully accountable.<br>Fully accessible. Completely willing to assume responsibility.</p>
	</div>
	</div><!-- This is the end of the third section of image  and its content. -->
</div><!-- this is the end of the Choose option 1 section. -->

<div class="dez-col-100 no-shadow">
	<div class="dez-col-70 no-shadow">
		<p class="device-center padding-left-20 padding-right-10 text-light-black">
			The work we do is serious. If you understand the responsibilities inherent in our business, embrace technology, understand that innovation is an imperative and also believe the soul of a  company is found in the ethics it practices, we invite you to join us.
		</p>
	</div>
	<div class="dez-col-20 padding-3 no-shadow">
		<center><a href="Career.php"><img src="content/about_bt_careers.jpg" alt="The patient took the prescibed medication" class="project-image-normal float-left" /></a></center>
	</div>
</div><!-- This is the end of the career button and its content. -->

<div class="dez-col-100 no-shadow text-light-black">
	<div class="dez-col-50 padding-left-25 padding-top-10 no-shadow">
	<center>
		<h2 class="special-text no-border mobile-font-4">Name 1</h2>
		<p><img src="content/about_arrow.jpg" alt="Medical abuse" class="project-image-vvsmall mobile-width-auto" /></p>
		<p class="text-light-green text-bold">CEO</p>
		<p class=" text-light-black">Please do not contact your physician's office with questions about an invoice from Acutis.<br>Contact us to discuss payment and our financial hardship program.</p>
	</center>
	</div>
</div>
<div class="dez-col-100 no-shadow">
	<div class="dez-col-40 padding-5 no-shadow">
	<center>
		<h2 class="special-text no-border mobile-font-4 text-bold">Name 1</h2>
		<p><img src="content/about_arrow.jpg" alt="OPIOIDS" class="project-image-vvsmall mobile-width-auto" /></p>
		<p class="text-light-green text-bold">Scientist</p>
		<p class=" text-light-black">Please do not contact your physician's office with questions about an invoice from Acutis.<br>Contact us to discuss payment and our financial hardship program.</p>
	</center>
	</div>
	<div class="dez-col-40 padding-5 no-shadow">
	<center>
		<h2 class="special-text no-border mobile-font-4 text-bold">Name 2</h2>
		<p><img src="content/about_arrow.jpg" alt="Narcotics" class="project-image-vvsmall mobile-width-auto" /></p>
		<p class="text-light-green text-bold">Lab Technician</p>
		<p class=" text-light-black">Please do not contact your physician's office with questions about an invoice from Acutis.<br>Contact us to discuss payment and our financial hardship program.</p>
	</center>
	</div>
</div>

<div class="dez-col-100 no-shadow">
	<div class="dez-col-40 padding-5 no-shadow">
	<center>
		<h2 class="special-text no-border mobile-font-4 text-bold">Name 3</h2>
		<p><img src="content/about_arrow.jpg" alt="Drug Abuse" class="project-image-vvsmall mobile-width-auto" /></p>
		<p class="text-light-green text-bold">Sales Specialist</p>
		<p class=" text-light-black">Please do not contact your physician's office with questions about an invoice from Acutis.<br>Contact us to discuss payment and our financial hardship program.</p>
	</center>
	</div>
	<div class="dez-col-40 padding-5 no-shadow">
	<center>
		<h2 class="special-text no-border mobile-font-4 text-bold">Name 4</h2>
		<p><img src="content/about_arrow.jpg" alt="drug test" class="project-image-vvsmall mobile-width-auto" /></p>
		<p class="text-light-green text-bold">Support Staff</p>
		<p class=" text-light-black">Please do not contact your physician's office with questions about an invoice from Acutis.<br>Contact us to discuss payment and our financial hardship program.</p>
	</center>
	</div>
</div>
		<div class="dez-col-100 background-soap-color no-shadow">
				<center class="padding-small">
				<img src="content/about_img_clia.jpg" alt="Pharmacogenetic test" class="cliaa" style="width:7.7%;" />
				<img src="content/about_img_NY-HEALTH.jpg" alt="pharmacogenetic testing" />
				</center>
		</div>

<?php include("footer.php"); ?>
<?php include("Bottom_Js.php"); ?>

</div>
</body>
</html>